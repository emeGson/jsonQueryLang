...
const UnsignedInt = (tokenizer: Tokenizer): Token | null => {
  ...
  const res = OneOrMore(tokenizer, (t: Tokenizer) => {
    return AnyOfString(t, "0123456789");
  });
  if (res === null) {
    tokenizer.reset(mark);
    return null;
...
const Float = (tokenizer: Tokenizer): Token | null => {
  ...
  return createToken(
    tokenizer,
    real.raw +
      (Array.isArray(decimal) ? decimal : []).map((t) => t.raw).join(""),
    "float",
  );
};
...
const String = (tokenizer: Tokenizer): Token | null => {
  ...
  const stringParser = (tok: Tokenizer) =>
    Choice(tok, [
      (t: Tokenizer) => StringLiteral(t, "\\'"),
      Any,
    ]);
...
const Identifier = (tokenizer: Tokenizer): Token | null => {
  ...
  return !body
    ? createToken(tokenizer, starter.raw, "identifier")
    : createToken(
      tokenizer,
      starter.raw + body.map((t) => t.raw).join(""),
      "identifier",
    );
};
...
const FunctionArguments = (tokenizer: Tokenizer): Token | null => {
  ...
  return createToken(
    tokenizer,
    res.map((r) => r.raw).join(""),
    "arguments",
    res.filter((r) =>
      r.type === "string" || r.type === "float" || r.type === "boolean" ||
      r.type === "expression"
    ),
  );
};
...
const Function = (tokenizer: Tokenizer): Token | null => {
  ...
  return createToken(
    tokenizer,
    func.map((r) => r.raw).join(""),
    "function",
    func.filter((r) =>
      r.type === "identifier" ||
      (r.type === "arguments" && r.children.length > 0)
    ),
  );
};
...
const Expression = (tokenizer: Tokenizer): Token | null => {
  ...
  return createToken(
    ...
    "expression",
    expression.filter((r) =>
      r.type === "identifier" || r.type === "function" || r.type === "wildcard"
    ),
  );
};
